...
#define	sigemptyset(mask)
#define	sigaddset(set,num)
...
static inline void DEBUG(char *format, ...)
{
	va_list ap;
	va_start(ap, format);
	if (sshfs.debug) vfprintf(stderr, format, ap);
	va_end(ap);
}
#endif
...
fprintf(stderr, "pty_expect_loop entered: fd %d %d\n", sshfs.fd, sshfs.ptyfd);
	while (1) {
#if 0
		struct pollfd fds[2];
		...
		if (len == passwd_len) {
			if (memcmp(buf, passwd_str, passwd_len) == 0) {
				send(sshfs.ptyfd, sshfs.password,
				      strlen(sshfs.password),0);
			}
...
static int start_ssh(void)
{
	...
	if (!CreatePipe(&sshfs.readfd, &pipeout, &sa, 0)) {
		fprintf(stderr, "cannot CreatePipe to shell\n");
		return -1;
	}
	if (!CreatePipe(&pipein, &sshfs.fd, &sa, 0)) {
		fprintf(stderr, "cannot CreatePipe to shell\n");
		return -1;
	}
	...
	for (i = 1; i < sshfs.ssh_args.argc; i++) {
		sprintf(p, " %s ", sshfs.ssh_args.argv[i]);
		p = cmdline + strlen(cmdline);
	}
	DEBUG("Createprocess: %s %s\n", sshfs.ssh_args.argv[0], cmdline);
	if (!CreateProcess(sshfs.ssh_args.argv[0], cmdline,
			NULL, NULL, TRUE, 0,
			NULL, NULL, &si, &pi)) {
		fprintf(stderr, "failed to execute '%s': %d\n",
			sshfs.ssh_args.argv[0], GetLastError());
		_exit(1);
	}
	CloseHandle(pipein);
	CloseHandle(pipeout);
	sshfs.plink = pi.hProcess;
...
static inline int writev(int fd, struct iovec *iov, size_t count)
{
	int i;
	DWORD ret;
	WSABUF wbuf[4];
	...
	if (!sshfs.directport) {
		...
		for (i = 0; i < count; i++) {
			...
			caddr_t buf = iov[i].iov_base;
			int res;

			while (len > 0) {
				if (!WriteFile(sshfs.fd, buf, len, &res,
									NULL))
					return -1;
				len -= res;
				buf += res;
	...
	for (i = 0; i < count; i++) {
		wbuf[i].len = iov[i].iov_len;
		wbuf[i].buf = iov[i].iov_base;
	}

	if (WSASend(fd, wbuf, count, &ret, 0, NULL, NULL) == 0)
		return (ret);
...
static inline char *WSAAPI gai_strerrorA(int ecode)
{
...
static int connect_to(char *host, char *port)
{
	...
	if (hostp)
		...
		hostp = host;
	err = getaddrinfo(hostp, port, &hint, &ai);
	if (err) {
...
static int do_read(struct buffer *buf)
{
	...
	while (size) {
		if (sshfs.directport)
			res = recv(sshfs.fd, p, size, 0);
		else if (!ReadFile(sshfs.readfd, p, size, &res, NULL))
			res = -1;
...
static void *WINAPI process_requests(void *data_)
{
	(void) data_;
	...
	if (!sshfs.reconnect) {
		/* harakiri */
		//FUSE_PORT_kill(_getpid(), SIGTERM);
	}
	return NULL;
}
...
static void random_string(char *str, int length)
{
	int i;
	for (i = 0; i < length; i++)
		*str++ = (char)('0' + rand(&sshfs.randseed) % 10);
	*str = '\0';
}
...
static sshfs_oper_init(void)
{
	sshfs_oper.oper.init       = sshfs_init;
	sshfs_oper.oper.getattr    = sshfs_getattr;
	sshfs_oper.oper.readlink   = sshfs_readlink;
	sshfs_oper.oper.mknod      = sshfs_mknod;
	sshfs_oper.oper.mkdir      = sshfs_mkdir;
	sshfs_oper.oper.symlink    = sshfs_symlink;
	sshfs_oper.oper.unlink     = sshfs_unlink;
	sshfs_oper.oper.rmdir      = sshfs_rmdir;
#undef	rename
	sshfs_oper.oper.rename     = sshfs_rename;
	sshfs_oper.oper.link       = sshfs_link;
	sshfs_oper.oper.chmod      = sshfs_chmod;
	sshfs_oper.oper.chown      = sshfs_chown;
	sshfs_oper.oper.truncate   = sshfs_truncate;
	sshfs_oper.oper.utime      = sshfs_utime;
	sshfs_oper.oper.open       = sshfs_open;
	sshfs_oper.oper.flush      = sshfs_flush;
	sshfs_oper.oper.fsync      = sshfs_fsync;
	sshfs_oper.oper.release    = sshfs_release;
	sshfs_oper.oper.read       = sshfs_read;
	sshfs_oper.oper.write      = sshfs_write;
	sshfs_oper.oper.statfs     = sshfs_statfs;
#if FUSE_VERSION >= 25
	sshfs_oper.oper.create     = sshfs_create;
	sshfs_oper.oper.ftruncate  = sshfs_ftruncate;
	sshfs_oper.oper.fgetattr   = sshfs_fgetattr;
#endif
	sshfs_oper.cache_getdir = sshfs_getdir;
}
...
static int read_password(void)
{
	...
	for (n = 0; n < max_password; n++) {
		...
		res = _read(0, &sshfs.password[n], 1);
		if (res == -1) {
...
//	ssh_add_arg("-oClearAllForwardings=yes");
	...
	TerminateProcess(sshfs.plink, 0);
	return res;
}
